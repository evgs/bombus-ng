...
MD5::MD5() {
	digestBitsLen=16;
	digestBits=new unsigned char[digestBitsLen];
	digestValid=false;
}
...
void MD5::MD5Update (const unsigned char *input, unsigned int inputLen) {
...
void MD5::updateArray(const unsigned char* input, int len, int offset) {
	MD5Update(&input[offset], len);
}
